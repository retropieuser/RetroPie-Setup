...
rp_module_repo="git https://github.com/flyinghead/flycast.git dev"
...
function configure_flycast() {
    ...
    for sys in "arcade" "dreamcast"; do
        ...
        addEmulator $def "$md_id" "$sys" "$md_inst/flycast %ROM%"
        addSystem "$sys"
    done
    ...
    # set to fullscreen at 1080p
    if [[ ! -f "$md_conf_root/dreamcast/flycast/emu.cfg" ]]; then
        cat >"$md_conf_root/dreamcast/flycast/emu.cfg" <<_EOF_
[window]
fullscreen = yes
height = 1080
maximized = no
